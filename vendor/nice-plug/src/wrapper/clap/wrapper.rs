...
use clap_sys::ext::track_info::{
    CLAP_EXT_TRACK_INFO, CLAP_EXT_TRACK_INFO_COMPAT, CLAP_TRACK_INFO_HAS_TRACK_NAME,
...
pub struct Wrapper<P: ClapPlugin> {
    ...
    track_name: AtomicRefCell<Option<String>>,
...
impl<P: ClapPlugin> Wrapper<P> {
    ...
    pub unsafe fn new(host_callback: *const clap_host) -> Arc<Self> {
        ...
        let wrapper = Self {
            ...
            track_name: AtomicRefCell::new(None),
    ...
    fn refresh_track_name(&self) {
        crate::nice_debug_assert!(self.is_main_thread());
        let track_name = {
            let host_track_info = self.host_track_info.borrow();
            host_track_info.as_ref().and_then(|host_track_info| {
                ...
                if !success || info.flags & CLAP_TRACK_INFO_HAS_TRACK_NAME == 0 {
                    return None;
                }
        ...
        let track_name = track_name.filter(|name| !name.trim().is_empty());
        if *self.track_name.borrow() != track_name {
            self.plugin
                .lock()
                .track_name_changed(track_name.as_deref());
            *self.track_name.borrow_mut() = track_name;
        }
    }
